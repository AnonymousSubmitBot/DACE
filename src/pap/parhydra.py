...
class ProblemEvaluator:
    ...
    def evaluate(self, smac_config: Configuration, seed: int = 1088):
        ...
        performance_matrix = copy.deepcopy(self.performance_matrix)
        performance_matrix.append(new_performance)
    ...
    def add_config_performance(self, config):
        config_str = "_".join(["{}".format(item) for item in config])
        new_performance = self.quality_history[config_str]
        self.performance_matrix.append(np.array(new_performance))


class PARHYDRA(BasePAP):
    def __init__(self, solver_class: Type[BaseSolver], solver_num: int = 4, config_batch_size: int = 100,
        ...
        self.logger = logzero.setup_logger(logfile=str(Path(os.path.dirname(os.path.abspath(__file__)),
                                                            "../../logs/PARHYDRA_logs/parhydra_run_{}_{}.log".format(
                                                                domain,
                                                                train_dim))),
                                           formatter=formatter,
                                           name="PARHYDRA Log for {} with dim={}".format(domain, train_dim),
    ...
    def update_config_pop(self):
        ...
        def smac_increase(try_index: int, result_dict):
            ...
            for c_index in range(self.solver_num):
                ...
                config = [best_config["{}".format(index)] for index in range(len(self.solver_class.config_range))]
                config_list.append(config)
                problem_evaluator.add_config_performance(config=config)
            result_dict["SMAC_RESULT_{}".format(try_index)] = (config_list, problem_evaluator.performance_matrix)
    ...
    def run(self, time_limit):
        ...
        while time.time() - st < time_limit * 3600:
            ...
            pickle.dump(experiment_record,
                        open("../../logs/PARHYDRA_logs/{}_{}.pkl".format(self.domain, self.train_dim), "wb"))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--distributed', type=strtobool, default=False, help='Run PARHYDRA in Distributed Mode')
    parser.add_argument('--time_limit', type=float, default=7, help='Time limit, unit is hour')
    args = parser.parse_args()

    parhydra = PARHYDRA(solver_class=BRKGASolver, config_batch_size=args.config_batch_size, solver_num=args.solver_num,
    ...
    parhydra.logger.info("PARHYDRA Configuration is: {}".format(json.dumps(vars(args), ensure_ascii=False, indent=2)))
    parhydra.run(args.time_limit)
    parhydra.terminate()
