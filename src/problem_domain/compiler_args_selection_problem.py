...
_candidate_train_index = [32, 17, 31, 29, 34, 39, 49, 13, 37,
                          11]  # 7, 8, 9, 10, 4, 5, 6, 3, 21,2, 16, 14, 0 cost so much time

_candidate_eval_index = [12, 1, 24, 36, 23, 40, 18, 22, 33, 41, 26, 19, 20, 25, 42, 28, 30, 45, 46, 47, 35, 44, 38, 43,
                         27, 48]  # 7, 8, 9, 10, 4, 5, 6, 3, 21,2, 16, 14, 0 cost so much time
...
class CompilerArgsSelectionProblem(BaseProblem):
    def __init__(self, dimension: int, train: bool = False, **kwargs):
        _candidate_program_index = _candidate_train_index if train else _candidate_eval_index
        ...
        self.polybench = self.program_dict["dataset"] != "cbench"

    def get_opt(self, opt_selection):
        """
        get compiler flag by solution
        """
        compile_flag = '-Os'
    ...
    def run_compiler(self, problem_dir, compile_flag):
        all_config: Dict[str] = json.load(open(Path(problem_dir, ".cm/meta.json"), "r"))
        source_files: List[str] = all_config["source_files"]
        compiler_vars = all_config["build_compiler_vars"]
        shell = ". /tmp_ck/CK/local/env/5f7f72fa5a44daa3/env.sh\n"
        if self.polybench:
            shell += ". /tmp_ck/CK/local/env/7697da46f8f05805/env.sh\n"
        shell += ". /tmp_ck/CK/local/env/bb90291a0a483a1b/env.sh\n. /tmp_ck/CK/local/env/5f7f72fa5a44daa3/env.sh 1\n"
        home_dir = os.path.expanduser("~")
        compile_pre = "$CK_CC -pipe $CK_FLAGS_CREATE_OBJ $CK_COMPILER_FLAGS_OBLIGATORY $CK_FLAGS_DYNAMIC_BIN ${CK_FLAG_PREFIX_INCLUDE}../ ${CK_FLAG_PREFIX_VAR}CK_HOST_OS_NAME2_LINUX=1 ${CK_FLAG_PREFIX_VAR}CK_HOST_OS_NAME_LINUX=1 ${CK_FLAG_PREFIX_VAR}CK_TARGET_OS_NAME2_LINUX=1 ${CK_FLAG_PREFIX_VAR}CK_TARGET_OS_NAME_LINUX=1 "
        compile_pre += " ".join(
            ["${{CK_FLAG_PREFIX_VAR}}{}{}{}".format(var, "=" if compiler_vars[var] != "" else "", compiler_vars[var])
             for
             var in compiler_vars])
        if self.polybench:
            compile_pre += " ${CK_FLAG_PREFIX_INCLUDE}\"" + home_dir + "/CK-TOOLS/lib-rtl-polybench-2012-gcc-11.4.0-linux-64/include\""
        compile_pre += " ${CK_FLAG_PREFIX_INCLUDE}\"" + home_dir + "/CK-TOOLS/lib-rtl-xopenme-0.3-gcc-11.4.0-linux-64/include\""
        output_pre = "$CK_CC $CK_COMPILER_FLAGS_OBLIGATORY  $CK_FLAGS_DYNAMIC_BIN"
        tmp_dir = Path(problem_dir,
                       "tmp_{}_{}_{}".format(str(int(time.time())), os.getpid(),
                                             "".join(random.sample('zyxwvutsrqponmlkjihgfedcba', 20))))
        ...
        # output binary
        shell += "\n".join(
            ["{0} {1} ../{2} ${{CK_FLAGS_OUTPUT}}{2}.o".format(compile_pre, compile_flag,
                                                               file) + "\ner=$?; if [ $er != 0 ]; then exit $er; fi\n"
             for file in
             source_files])

        # output target
        # shell += "\n{} {} ${{CK_FLAGS_OUTPUT}}a.out $CK_LD_FLAGS_MISC $CK_LD_FLAGS_EXTRA ".format(output_pre, " ".join(
        #     ["{}.o".format(file) for file in source_files]))
        # if self.polybench:
        #     shell += "${CK_FLAG_PREFIX_LIB_DIR}\"" + home_dir + "/CK-TOOLS/lib-rtl-polybench-2012-gcc-11.4.0-linux-64/lib\" -lrtlpolybench  "
        # shell += "${CK_FLAG_PREFIX_LIB_DIR}\"" + home_dir + "/CK-TOOLS/lib-rtl-xopenme-0.3-gcc-11.4.0-linux-64/lib\" -lrtlxopenme $CK_EXTRA_LIB_M\n"
        # shell += "\ner=$?; if [ $er != 0 ]; then exit $er; fi\n"
        # print(shell)
        file_name = "tmp_{}.sh".format("".join(random.sample('zyxwvutsrqponmlkjihgfedcba', 20)))
        open(Path(tmp_dir, file_name), "w").write(shell)
        cmd = 'bash -c "chmod 755 ./{0}; . ./{0}"'.format(file_name)
        ...
        if rc != 0:
            ...
            result = np.sum([os.path.getsize(Path(tmp_dir, "{}.o".format(file))) for file in source_files])
        shutil.rmtree(tmp_dir)
        return result

    def get_compile_size_new(self, program, compile_flag):
        temp_path = os.path.abspath(os.curdir)
        problem_dir = Path("/tmp_ck", program)
        result = self.run_compiler(problem_dir, compile_flag)
        os.chdir(temp_path)
        return result

    @staticmethod
    def get_compile_size_old(program, compile_flag):
        """Test the compiled executable file size against the program name and compilation options"""
        proj_tmp_path = Path(os.path.dirname(os.path.abspath(__file__)), "../../tmp")
        temp_path = os.path.abspath(os.curdir)
    ...
    def evaluate(self, solution: Union[NpArray, List[Union[NpArray, List[int]]], Tensor]):
        if not isinstance(solution, NpArray):
        ...
        # self.get_compile_size_old(program, compile_flag)
        return -self.get_compile_size_new(program, compile_flag)
