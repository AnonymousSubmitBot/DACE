...
def evaluate_problem_with_config(vae_params, domain, vae_weights, ins_emb, gpu_index,
    ...
    problem = SurrogateProblem(dimension=vae_params['in_dim'], domain=domain, vae_model=SurrogateVAE(**vae_params),
...
class DACE(BasePAP):
    def __init__(self, solver_class: Type[BaseSolver], solver_num: int = 4, max_iter: int = 4, temp_num: int = 4,
        ...
        self.logger = logzero.setup_logger(logfile=str(Path(os.path.dirname(os.path.abspath(__file__)),
                                                            "../../logs/DACE_logs/dace_run_{}_{}.log".format(domain,
                                                                                                             train_dim))),
                                           formatter=formatter,
                                           name="DACE Log for {} with dim={}".format(domain, train_dim),
        ...
        self.ins_embs: List[NpArray] = [model_dict['instance_embedding_{}'.format(index)].cpu().detach().numpy() for
    ...
    def run(self, load_pickle: bool = False):
        ...
        if load_pickle:
            experiment_record = pickle.load(
                open("../../logs/DACE_logs/{}_{}.pkl".format(self.domain, self.train_dim), "rb"))
            ...
            pickle.dump(experiment_record,
                        open("../../logs/DACE_logs/{}_{}.pkl".format(self.domain, self.train_dim), "wb"))
    ...
    def only_config_update(self, load_pickle: bool = False):
        ...
        experiment_record["update_history"] = []
        pickle.dump(experiment_record,
                    open("../../logs/DACE_logs/{}_{}_only_update_config.pkl".format(self.domain, self.train_dim), "wb"))
...
if __name__ == '__main__':
    ...
    # parser.add_argument('--distributed', type=strtobool, default=False, help='Run DACE in Distributed Mode')
    ...
    dace = DACE(gpu_indices=[0, 1, 2, 3, 4, 5, 6, 7], solver_class=BRKGASolver, solver_max_eval=args.solver_max_eval,
                smac_max_try=args.smac_max_try, temp_num=args.temp_num, domain=args.problem_domain,
                train_dim=args.problem_dim, max_parallel_num=args.max_parallel_num, es_epoch_num=args.mutator_epoch_num,
                es_pop_size=args.mutator_pop_size, max_iter=args.max_iter, solver_num=args.solver_num,
                add_recommend_config_set=args.add_recommend_config_set,
                fixed_initial_config_set=args.fixed_initial_config_set)
    dace.logger.info("DACE Configuration is: {}".format(json.dumps(vars(args), ensure_ascii=False, indent=2)))
